// Code generated by goplus from runtime_java.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func failureKindText(kind FailureKind) string {
	switch __gp_m0 := any(kind).(type) {
	case nil:
		//goplus:pattern UnexpectedByte()
		return "unexpected byte"
	case nil:
		//goplus:pattern ExpectedIdentifier()
		return "expected identifier"
	case nil:
		//goplus:pattern ExpectedEquals()
		return "expected '='"
	case nil:
		//goplus:pattern ExpectedInteger()
		return "expected integer"
	case nil:
		//goplus:pattern IntegerOverflow()
		return "integer overflow"
	case nil:
		//goplus:pattern AmbiguousFirstSet()
...
func failureAt(kind FailureKind, input string, offset, lineStart, line int, expected string) *Failure {
	unexpected := "EOF"
	if offset < len(input) {
		unexpected = "'" + string([]byte{input[offset]}) + "'"
	}
