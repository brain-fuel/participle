...
func failureKindText(kind FailureKind) string {
	match kind {
	case UnexpectedByte(): return "unexpected byte"
	...
	case AmbiguousFirstSet(): return "ambiguous FIRST set"
	}
}
...
func parseAssignments(input string) ([]Assignment, *Failure) {
	...
	for {
		...
		assignments = append(assignments, Assignment{Name: name, Value: signed, Span: spanValue(start, end, 3)})
...
func skipSpace(input string, offset, lineStart, line int) (int, int, int) {
	for offset < len(input) {
		switch input[offset] {
		case ' ', '\t', '\r', ';': offset++
		case '\n': offset++; line++; lineStart = offset
		default: return offset, lineStart, line
		}
	}
...
func skipHorizontal(input string, offset, lineStart, line int) (int, int, int) {
	for offset < len(input) && (input[offset] == ' ' || input[offset] == '\t') { offset++ }
	return offset, lineStart, line
}
